// src/views/AddActualMaterialScreen.tsx
...
export default function AddActualMaterialScreen({ route }: Props) {
  ...
  const submit = async () => {
    ...
    if (!itemnum.trim()) { showError('Item obligatoire'); return; }

    const q = Number(quantity || 0);
    if (Number.isNaN(q) || q <= 0) { showError('Quantité invalide (doit être > 0)'); return; }
