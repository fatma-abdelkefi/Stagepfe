// src/views/DetailsMaterialsScreen.tsx
...
type Props = {
  route: RouteProp<RootStackParamList, 'DetailsMaterials'>;
};

type NavigationProp = NativeStackNavigationProp<RootStackParamList, 'DetailsMaterials'>;
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Matériel - #{workOrder?.wonum}</Text>
        <View style={{ width: 44 }} />
      </View>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        {materials.length === 0 ? (
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>Aucun matériel utilisé</Text>
          </View>
        ) : (
            ...
            {materials.map((item: any, index: number) => (
              <View key={index} style={styles.itemCard}>
                <View style={styles.itemHeader}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={styles.title}>{item.itemnum || 'N/A'}</Text>
                    {item.quantity && <Text style={styles.subtitle}>Quantité : {item.quantity}</Text>}
                  </View>
                </View>
