// src/screens/DetailsActualLaborScreen.tsx
...
type NavProp = NativeStackNavigationProp<RootStackParamList, 'DetailsActualLabor'>;

export default function DetailsActualLaborScreen({ route }: Props) {
  const navigation = useNavigation<NavProp>();
  const workOrder = route.params.workOrder as any;

  const data = useMemo(() => (workOrder?.actualLabor ?? []) as { laborcode: string; regularhrs: number }[], [workOrder]);
    ...
    <SafeAreaView style={styles.container}>
      <LinearGradient colors={['#005ed1', '#0ea5e9']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.header}>
        <View style={styles.headerRow}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={styles.title}>Main d'œuvre réelle</Text>
      ...
      {data.length === 0 ? (
        <View style={styles.empty}>
          ...
          <Text style={styles.emptyText}>Aucune main d'œuvre réelle.</Text>
        </View>
      ) : (
        <FlatList
          data={data}
          ...
          renderItem={({ item }) => (
            <View style={styles.card}>
              <View style={styles.row}>
                ...
                <Text style={styles.value}>{item.laborcode || '-'}</Text>
              </View>
              <View style={styles.row}>
                <Text style={styles.label}>Heures</Text>
                <Text style={styles.value}>{Number(item.regularhrs || 0).toFixed(2)}</Text>
              </View>
            </View>
          )}
        />
