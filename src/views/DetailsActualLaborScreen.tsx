...
import { getActualsFromWoHref, normalizeOslcHref, type ActualLaborItem } from '../services/workOrderDetailsService';
...
function normalizeHref(href: any): string {
  ...
  if (!u) return '';
  return normalizeOslcHref(u).replace(/\/+$/, '');
}
...
export default function DetailsActualLaborScreen({ route }: Props) {
  ...
  // ✅ Use woHref param if passed, else workOrder.href
  const woHrefParam = safeTrim((route.params as any)?.woHref);
  const woHrefFromWO = safeTrim((workOrder as any)?.href);

  const woHref = useMemo(() => {
    const chosen = woHrefParam || woHrefFromWO;
    const normalized = normalizeHref(chosen);
    return normalized;
  }, [woHrefParam, woHrefFromWO]);
  ...
  const fetchActualLabor = useCallback(
    async (mode: 'initial' | 'refresh') => {
      try {
        ...
        console.log('[DetailsActualLabor] fetched count:', res.actualLabor?.length ?? 0);
  ...
  const totalHours = useMemo(() => items.reduce((sum, x) => sum + (Number(x.regularhrs) || 0), 0), [items]);

  return (
    <SafeAreaView style={detailsStyles.container}>
      <DetailsHeader title="Main d'œuvre réelle" subtitle={`OT #${wonum || '-'} • Total: ${totalHours.toFixed(2)} h`} />

      <ScrollView
        style={detailsStyles.content}
        showsVerticalScrollIndicator={false}
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => fetchActualLabor('refresh')} />}
      >
              ...
              <View key={index} style={detailsStyles.card}>
                <View style={styles.itemHeader}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={detailsStyles.subtitle}>Heures : {Number(item.regularhrs || 0)} h</Text>
                    {!!item.transdate && (
                      <Text style={[detailsStyles.subtitle, { marginTop: 4 }]}>
                        Date : {formatDateTime(item.transdate)}
                      </Text>
                    )}
                  </View>
                </View>
      ...
      <ErrorModal visible={errorVisible} title="Erreur" message={errorMessage} onClose={() => setErrorVisible(false)} />
    </SafeAreaView>
  );
}
